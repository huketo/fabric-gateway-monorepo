...
function networkUp() {
  # check if docker network(fabric_network) exists
  if [ -z "$(${CONTAINER_CLI} network ls --filter name=fabric_network -q)" ]; then
    infoln "Creating network 'fabric_network'"
    ${CONTAINER_CLI} network create fabric_network
  fi
...
function networkDown() {
  # Remove docker network(fabric_network)
  if [ -n "$(${CONTAINER_CLI} network ls --filter name=fabric_network -q)" ]; then
    infoln "Removing network 'fabric_network'"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer0.example.com docker_orderer1.example.com docker_orderer2.example.com docker_peer0.org1.example.com docker_peer1.org1.example.com docker_peer0.org2.example.com docker_peer1.org2.example.com docker_peer0.org3.example.com docker_peer1.org3.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf channel-artifacts log.txt *.tar.gz caliper.yaml'
  fi
}
